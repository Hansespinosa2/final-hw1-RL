...
# YOUR IMPLEMENTATION HERE #
    while True:
        ...
        for s, a in enumerate(policy):
            ...
            value_function[s] = reward + gamma * prob * prev_value_function[next_s]
            
        if np.max(abs(value_function-prev_value_function)) < tol:
            break
	############################
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    for s in range(nS):
        ...
        for a in range(nA):
            prob, next_s, reward, terminal = P[s][a][0]
            Q[s][a] = reward + gamma * prob * value_from_policy[next_s]
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while i==0 or np.linalg.norm(policy - old_policy,1) > 0:
        old_policy = policy.copy()
        value_function = policy_evaluation(P,nS,nA,old_policy,gamma,tol)
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while True:
        ...
        for s in range(nS):
            ...
            for a in range(nA):
                prob, next_s, reward, terminal = P[s][a][0]
                values.append(reward + gamma * prob * old_value_function[next_s])
            value_function[s] = max(values)
